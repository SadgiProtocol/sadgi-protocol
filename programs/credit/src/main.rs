...
use sadgi_types::program::{
    CanonicalJournal, IssuerVerifier, MockVerifier, PrivateClaim, PublicRequirement,
};
...
fn main() {
    ...
    if !MockVerifier::verify_claim(
        private_claim.value,
        &private_claim.issuer_signature,
        &issuer_pubkey,
    ) {
        panic!("InvalidSignature");
    }

    // 4. Evaluate Threshold (Credit Score >= 700)
    ...
    let journal = CanonicalJournal {
        program_id,
        subject_pubkey,
        claim_type: Symbol::new(&soroban_env, "CreditScore"),
        result: true,
        timestamp,

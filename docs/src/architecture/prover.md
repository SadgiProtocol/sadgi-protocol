...
## 3. zkVM Execution (RISC Zero)
The Prover instantiates the RISC Zero zkVM locally. It feeds the `caller_contract_id` and any public inputs into the VM, and executes the binary.
...
The Prover serializes these into a `SadgiReceipt` according to SIP-1, constructs a Soroban transaction, signs it with its Stellar private key, and submits it to the Compute Marketplace to claim the XLM bounty.

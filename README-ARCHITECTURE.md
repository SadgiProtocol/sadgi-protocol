...
A `no_std` Rust library that allows developers to easily construct `JobRequest` structures and define the cryptographic constraints (e.g., RISC Zero Image IDs) required for their programs.
...
The off-chain daemon operated by infrastructure providers. It constantly indexes the Stellar ledger for new `JobRequest` events, executes the target RISC-V binary, generates a `SadgiReceipt`, and submits the transaction back to the Marketplace.
